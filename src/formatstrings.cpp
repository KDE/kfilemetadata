...
{
    KFormat form;
    return i18nc("@label bitrate (per second)",
                 "%1/s",
                 form.formatValue(value.toInt(),
                                  KFormat::Unit::Bit,
                                  threeSignificantDigits(value.toInt()),
                                  KFormat::UnitPrefix::AutoAdjust,
                                  KFormat::MetricBinaryDialect));
}

QString FormatStrings::formatSampleRate(const QVariant &value)
{
    KFormat form;
    return form.formatValue(value.toInt(),
                            KFormat::Unit::Hertz,
                            threeSignificantDigits(value.toInt()),
                            KFormat::UnitPrefix::AutoAdjust,
                            KFormat::MetricBinaryDialect);
...
QString FormatStrings::formatOrientationValue(const QVariant &value)
{
    ...
    switch (value.toInt()) {
    case 1:
        string = i18nc("Description of image orientation", "Unchanged");
        break;
    case 2:
        string = i18nc("Description of image orientation", "Horizontally flipped");
        break;
    case 3:
        string = i18nc("Description of image orientation", "180° rotated");
        break;
    case 4:
        string = i18nc("Description of image orientation", "Vertically flipped");
        break;
    case 5:
        string = i18nc("Description of image orientation", "Transposed");
        break;
    case 6:
        string = i18nc("Description of image orientation, counter clock-wise rotated", "90° rotated CCW ");
        break;
    case 7:
        string = i18nc("Description of image orientation", "Transversed");
        break;
    case 8:
        string = i18nc("Description of image orientation, counter clock-wise rotated", "270° rotated CCW");
        break;
    default:
        break;
    }
...
QString FormatStrings::formatPhotoFlashValue(const QVariant &value)
{
    ...
    const QMap<int, QString> flashTranslation = {{0x00, i18nc("Description of photo flash", "No flash")},
...
QString FormatStrings::formatPhotoExposureBias(const QVariant &value)
{
    ...
    if (integral != 0) {
        return i18nc("Exposure compensation given as integral with fraction, in exposure value (EV)",
                     "%1 %2/%3 EV",
                     locale.toString(integral),
                     locale.toString(abs(nominator)),
                     locale.toString(denominator));
    }
    return i18nc("Exposure compensation given as rational, in exposure value (EV)", "%1/%2 EV", locale.toString(nominator), locale.toString(denominator));
}
